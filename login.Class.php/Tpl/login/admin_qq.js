ELi['login/admin_qq'] = {
    url: dir + Plus + FENGE + "login" + FENGE + "admin_qq" + FENGE,
    init: function () {
        ELi['login/admin_qq'].get();
    },
    edit(OBJ, diercai) {
        if (diercai != "add") {
            diercai = "put";
        ...
        BIAO = [
            ...
            'openid($$)openid($$)textshow($$)($$)openid($$)' + D.openid
        ...
        html += jsfrom('ff' + $class + '($$)($$)submit($$)($$)layer.close(OPID);ELi[\'login/admin_qq\'].init()($$)提交($$)tijiao');
        html += "</form>";
        OPID = layer.open({
            type: 1,
            zIndex: 10000,
        ...
        layui.form.on('submit(ff' + $class + ')', function (formdata) {
            ...
            apptongxin(ELi['login/admin_qq'].url + TOKEN + FENGE + diercai, formdata, function (data) {
    ...
    del(OBJ) {
        ...
        apptongxin(ELi['login/admin_qq'].url + TOKEN + FENGE + 'del', fromdata, function (data) {
            if (data.token && data.token != "") {
                TOKEN = data.token;
            }
            if (data.code == 99) {
    ...
    get() {
        ...
        layui.table.on('toolbar(user' + $class + ')', function (obj) {
            if (obj.event === 'add') {
                ELi['login/admin_qq'].add();
            } else if (obj.event === 'refresh') {
                layer.closeAll();
                ELi['login/admin_qq'].init();
        ...
        layui.table.on('tool(user' + $class + ')', function (obj) {
            if (obj.event === 'del') {
                layer.confirm('真的要删除吗?', function (index) {
                    ELi['login/admin_qq'].del(obj);
                    layer.close(index);
                });
            } else if (obj.event === 'edit') {
                ELi['login/admin_qq'].edit(obj);
            }
        });
