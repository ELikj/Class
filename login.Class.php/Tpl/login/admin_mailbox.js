ELi['login/admin_mailbox'] = {
    url: dir + Plus + FENGE + "login" + FENGE + "admin_mailbox" + FENGE,
    init: function () {
        ELi['login/admin_mailbox'].get();
    },
    edit(OBJ, diercai) {
        if (diercai != "add") {
            diercai = "put";
        ...
        BIAO = [
            ...
            'uid($$)UID($$)text($$)($$)($$)' + D.uid,
            'mailbox($$)登陆邮箱($$)text($$)($$)登陆邮箱($$)' + D.mailbox,
        ...
        html += jsfrom('ff' + $class + '($$)($$)submit($$)($$)layer.close(OPID);ELi[\'login/admin_mailbox\'].init()($$)提交($$)tijiao');
        html += "</form>";
        OPID = layer.open({
            type: 1,
            zIndex: 10000,
        ...
        layui.form.on('submit(ff' + $class + ')', function (formdata) {
            ...
            apptongxin(ELi['login/admin_mailbox'].url + TOKEN + FENGE + diercai, formdata, function (data) {
    ...
    del(OBJ) {
        ...
        apptongxin(ELi['login/admin_mailbox'].url + TOKEN + FENGE + 'del', fromdata, function (data) {
            if (data.token && data.token != "") {
                TOKEN = data.token;
            }
            if (data.code == 99) {
    ...
    add() {
        ELi['login/admin_mailbox'].edit({
            data: {"id":0,"uid":"0","mailbox":"","password":"","off":"0"}
        }, 'add');
    ...
    get() {
        ...
        layui.table.on('toolbar(user' + $class + ')', function (obj) {
            if (obj.event === 'add') {
                ELi['login/admin_mailbox'].add();
            } else if (obj.event === 'refresh') {
                layer.closeAll();
                ELi['login/admin_mailbox'].init();
        ...
        layui.table.on('tool(user' + $class + ')', function (obj) {
            if (obj.event === 'del') {
                layer.confirm('真的要删除吗?', function (index) {
                    ELi['login/admin_mailbox'].del(obj);
                    layer.close(index);
                });
            } else if (obj.event === 'edit') {
                ELi['login/admin_mailbox'].edit(obj);
            }
        });
