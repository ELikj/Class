...
if($kongzhi == 'get'){
    ...
    unset($_POST['id']);
    if($_POST['findback'] != ""){
        $_POST['findback'] = ELimm($_POST['findback'] );
    }else{
        unset($_POST['findback']);
    }
    if($_POST['password'] != ""){
    ...
    if(strlen($_POST['account']) < 4){
        return echoapptoken([],-1,'账号格式错误',$SESSIONtoken);
    }
    ...
    $fan = $db ->where(['account' => $_POST['account'] ])->find();
    if($fan){
        return echoapptoken([],-1,'用户名存在',$SESSIONtoken);
    }
