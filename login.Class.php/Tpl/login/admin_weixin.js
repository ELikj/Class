ELi['login/admin_weixin'] = {
    url: dir + Plus + FENGE + "login" + FENGE + "admin_weixin" + FENGE,
    init: function () {
        ELi['login/admin_weixin'].get();
    },
    edit(OBJ, diercai) {
        if (diercai != "add") {
            diercai = "put";
        ...
        BIAO = [
            ...
            'unionid($$)UnionID($$)textshow($$)($$)UnionID机制($$)' + D.unionid,
            'openid($$)公众号($$)textshow($$)($$)公众号($$)' + D.openid,
            'openidx($$)小程序($$)textshow($$)($$)小程序($$)' + D.openidx,
            'openidg($$)小游戏($$)textshow($$)($$)小游戏($$)' + D.openidg,
            'openido($$)开放平台($$)textshow($$)($$)开放平台($$)' + D.openido
        ...
        html += jsfrom('ff' + $class + '($$)($$)submit($$)($$)layer.close(OPID);ELi[\'login/admin_weixin\'].init()($$)提交($$)tijiao');
        html += "</form>";
        OPID = layer.open({
            type: 1,
            zIndex: 10000,
        ...
        layui.form.on('submit(ff' + $class + ')', function (formdata) {
            ...
            apptongxin(ELi['login/admin_weixin'].url + TOKEN + FENGE + diercai, formdata, function (data) {
    ...
    del(OBJ) {
        ...
        apptongxin(ELi['login/admin_weixin'].url + TOKEN + FENGE + 'del', fromdata, function (data) {
            if (data.token && data.token != "") {
                TOKEN = data.token;
            }
            if (data.code == 99) {
    ...
    add() {
        ELi['login/admin_weixin'].edit({
            data: {"id":0,"uid":"0","weixin":"","password":"","off":"0"}
        }, 'add');
    ...
    get() {
        $("#LAY_app_body ." + $class).html('<style> .' + $class + ' .qfys0{color:#FF5722;} .' + $class + ' .qfys1{color:#009688;}.' + $class + ' .qfys2{color:green;}.' + $class + ' .qfys3{color:#1E9FFF;}</style><div class="layui-fluid"><div class="layui-card"><div class="layui-card-body" style="padding: 15px;">'
            +
            '<div class="' + $class + 'saixuan" style="display:none;margin-bottom:8px;"><form name="form" class="layui-form"> <div class="layui-inline" style="width:88px;"> <input class="layui-input so_var" name="so_uid" autocomplete="off" placeholder="用户UID"> </div> <div class="layui-inline"> <input class="layui-input so_var" name="so_unionid" autocomplete="off" placeholder="unionid"> </div>  <div class="layui-inline"> <input class="layui-input so_var" name="so_openid" placeholder="公众号" autocomplete="off"> </div>  <div class="layui-inline"> <input class="layui-input so_var" name="so_openidx" placeholder="小程序" autocomplete="off"> </div> <div class="layui-inline"> <input class="layui-input so_var" name="so_openidg" placeholder="小游戏" autocomplete="off"> </div> <div class="layui-inline"> <input class="layui-input so_var" name="so_openido" placeholder="开放平台" autocomplete="off"> </div>  <button class="layui-btn" lay-event="sousuo" lay-submit lay-filter="tijiao' + $class + '">搜索</button></form> </div>'
            +
            '<table class="layui-hide" id="user' + $class + '" lay-filter="user' + $class + '"></table></div></div></div>');
        layui.table.render({
            elem: '#user' + $class,
            url: ELi['login/admin_weixin'].url + TOKEN + FENGE + 'get',
        ...
        layui.table.on('toolbar(user' + $class + ')', function (obj) {
            if (obj.event === 'add') {
                ELi['login/admin_weixin'].add();
            } else if (obj.event === 'refresh') {
                layer.closeAll();
                ELi['login/admin_weixin'].init();
        ...
        layui.table.on('tool(user' + $class + ')', function (obj) {
            if (obj.event === 'del') {
                layer.confirm('真的要删除吗?', function (index) {
                    ELi['login/admin_weixin'].del(obj);
                    layer.close(index);
                });
            } else if (obj.event === 'edit') {
                ELi['login/admin_weixin'].edit(obj);
            }
        });
